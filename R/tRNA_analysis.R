...
featureCounts_PA_sense_antisense_df_t_rel_L <- data.frame(rbind(featureCounts_PA_sense_df_t_rel_L_NN2,
                                                                featureCounts_PA_antisense_df_t_rel_L_NN2,
                                                                featureCounts_PA_sense_df_t_rel_L_SG17M,
                                                                featureCounts_PA_antisense_df_t_rel_L_SG17M))

featureCounts_PA_sense_antisense_df_t_rel_L$type2 <- with(featureCounts_PA_sense_antisense_df_t_rel_L,
                                                          ifelse(type == "CDS.undefined", "CDS (undefined)",
                                                                 ifelse(type == "ncRNA.undefined", "nCDS (undefined)",
                                                                        type)))
amount_rna <-
  ggplot(featureCounts_PA_sense_antisense_df_t_rel_L) +
  geom_col(aes(x=featureType, y=abundance, fill=reorder(type2, +abundance))) +
  facet_wrap(~strand_type) +
  theme_pubr(border=TRUE, base_size=11) + 
  theme(legend.title = element_blank(), legend.position = "top") + 
  xlab(" ") + ylab("Relative abundance\n\n") +
  scale_fill_manual(values=c("darkblue", "orange", "cyan4", "chartreuse4", "black")) + coord_flip()
...
featureCounts_PA_antisense_df_t_2_L$RNA_type2 <- with(featureCounts_PA_antisense_df_t_2_L,
                                                      ifelse(RNA_type == "ncRNA", "nCDS", "CDS"))

featureCounts_PA_sense_df_t_2_L$RNA_type2 <- with(featureCounts_PA_sense_df_t_2_L,
                                                      ifelse(RNA_type == "ncRNA", "nCDS", "CDS"))

quantify_antisense_RNA <-
  ggplot(featureCounts_PA_antisense_df_t_2_L, aes(x=RNA_type2, y=log10(abundance))) +
  geom_violin() + 
  geom_jitter(aes(colour=time), width = 0.05) + 
  stat_summary(fun.data = "mean_sdl",  fun.args = list(mult = 1), 
               geom = "pointrange", color = "red") +
  stat_compare_means(label = "p.signif", size=6, label.x.npc = "center", label.y = 6.2) + 
  theme_pubr(border=TRUE, base_size=11) + ylim(1,7) +xlab(" ") + 
  ylab("\n\n\n\n\n\n\nAbundance (log10 scale)") + facet_wrap(~strain) + 
  theme(legend.position = "none", axis.text.x = element_text(size=10)) +
  scale_color_manual(values=c("black","orange"))


quantify_sense_RNA <-
  ggplot(featureCounts_PA_sense_df_t_2_L, aes(x=RNA_type2, y=log10(abundance))) +
  geom_violin() + 
  geom_jitter(aes(colour=time), width = 0.05) + 
  stat_summary(fun.data = "mean_sdl",  fun.args = list(mult = 1), 
               geom = "pointrange", color = "red") +
  stat_compare_means(label = "p.signif", size=6, label.x.npc = "center", label.y = 6.2) + 
  theme_pubr(border=TRUE, base_size=11) + ylim(1,7) +ylab("\n\n\n") + xlab(" ") + facet_wrap(~strain) + 
  theme(legend.position = "none", axis.text.x = element_text(size=10)) +
  scale_color_manual(values=c("black","orange"))

merge_quantify <- ggarrange(quantify_antisense_RNA, 
                            quantify_sense_RNA, 
...
featureCounts_PA_sense_variance <- data.frame(rbind(featureCounts_PA_sense_df_t_NN2, featureCounts_PA_sense_df_t_SG17M))
featureCounts_PA_sense_variance$logCDS <- log10(featureCounts_PA_sense_variance$CDS)
featureCounts_PA_sense_variance$logCDS.undefined <- log10(featureCounts_PA_sense_variance$CDS.undefined)
featureCounts_PA_sense_variance$ncRNA.undefined <- log10(featureCounts_PA_sense_variance$ncRNA.undefined)
featureCounts_PA_sense_variance$tRNA <- log10(featureCounts_PA_sense_variance$tRNA)
featureCounts_PA_sense_variance$rRNA <- log10(featureCounts_PA_sense_variance$rRNA)

featureCounts_PA_sense_variance_log <- select(featureCounts_PA_sense_variance, 
                                              c(logCDS, logCDS.undefined, ncRNA.undefined,
                                                tRNA, rRNA, mixed, strandType))
hc_sense <- hclust(dist(featureCounts_PA_sense_variance_log))
ddata_sense <- dendro_data(hc_sense, type = "rectangle")
featureCounts_PA_antisense_df_t_NN2
...
featureCounts_PA_antisense_variance$logCDS <- log10(featureCounts_PA_antisense_variance$CDS)
featureCounts_PA_antisense_variance$logCDS.undefined <- log10(featureCounts_PA_antisense_variance$CDS.undefined)
featureCounts_PA_antisense_variance$log10ncRNA.undefined <- log10(featureCounts_PA_antisense_variance$ncRNA.undefined)
featureCounts_PA_antisense_variance$log10tRNA <- log10(featureCounts_PA_antisense_variance$tRNA)
# featureCounts_PA_antisense_variance$rRNA <- log10(featureCounts_PA_antisense_variance$rRNA)
...
res.var.antisense <- data.frame(res.var.antisense$cos2)        
res.var.antisense <- res.var.antisense[order(-res.var.antisense$Dim.1),]
res.var.antisense <- data.frame(res.var.antisense)

res.var.antisense.sub <- select(res.var.antisense, c("Dim.1", "Dim.2", "Dim.3"))
...
res.var.sense <- data.frame(res.var.sense$cos2)        
res.var.sense <- res.var.sense[order(-res.var.sense$Dim.1),]
res.var.sense <- data.frame(res.var.sense)

res.var.sense.sub <- select(res.var.sense, c("Dim.1", "Dim.2", "Dim.3"))
...
res.var.both.L <- gather(res.var.both, key="Dim", value="PCA", -c("type", "strand"))
...
variance_antisense_pca_plot <- 
  ...
  theme_pubr(border=TRUE, legend="bottom", base_size=11) + ylim(-0.8, 0.8) + xlim(-3, 3) +
  scale_color_manual(values=c("darkred", "darkblue", "orange", "black")) +
  theme(legend.position = "none") + 
  geom_label(aes(x=-1.8, y = 0.7, label = "Antisense"), fill="white")
...
variance_sense_pca_plot <- ggplot(pcaInd_sense_1_ind$data) + 
  ...
  scale_color_manual(values=c("darkred", "darkblue", "orange", "black")) +
  theme(legend.position = "none") + 
  geom_label(aes(x=-1.8, y = 0.7, label = "Sense"), fill="white")

res.var.both.L$type2 <- with(res.var.both.L, ifelse(type == "CDS.undefined", "CDS (undefined)",
                                                                 ifelse(type == "ncRNA.undefined", "nCDS (undefined)",
                                                                        type)))

variance_both <- ggplot(res.var.both.L) + 
  geom_col(aes(x=Dim, y=PCA, fill=reorder(type2, PCA))) + facet_grid(~strand) + theme_pubr(base_size=11, border=TRUE) + 
  theme(legend.title = element_blank(), legend.position = "top", 
        axis.text.x = element_text(angle = 90, vjust = 1, hjust=1)) + 
  xlab(" ") + ylab("Relative abundance") + coord_flip() +
  scale_fill_manual(values=c("black", "chartreuse4", "cyan4", "darkblue", "orange"))

ab <- ggarrange(variance_antisense_pca_plot, variance_sense_pca_plot, labels=c("A", "B"))
ab2 <- ggarrange(ab, variance_both, nrow = 2, heights = c(1,0.3), labels=c("A", "C"))

#ggsave(ab2, filename = "save_figures/rnA_abundance_variance.tif", 
 #      device="tiff", dpi=600, units="cm", width=17.4, height=21.6)
