...
setwd("/ngsssd1/tuem_mp/projects_MariePust_2018/rna_ont_nn2_sg17m_2021/MinionExperiments202009/d_Ranalysis")

# Load input ####
input_bam <- list.files(
  path = "/ngsssd1/tuem_mp/projects_MariePust_2018/rna_ont_nn2_sg17m_2021/MinionExperiments202009/b_minimap2/",
  pattern = ".bam", full.names = TRUE)

UserDefinedAnnotationRef <- "/mnt/sfb900nfs/groups/tuemmler/mariep/refseqs/PA_NN2_SG17M_C2/NN2_SG17M_CS_curated.gtf"
Ref_genome <- "/mnt/sfb900nfs/groups/tuemmler/mariep/refseqs/PA_NN2_SG17M_C2/NN2_SG17M_CS.fna"
...
a <- ggarrange(sense_pca_plot, antisense_pca_plot, nrow=1, ncol=2, labels=c("A", "B"),
               common.legend = TRUE, font.label = list(color = "black", size=8),
               legend = "bottom")
...
ggsave(a3, file="save_figures/Figure_08.tif", 
       device="tiff", width=12, height=12, dpi=300, units = "cm", bg="white")
...
rownames(logcpm_sense_df_h) <- c("rihB", "prpL", "920454", 
                                 "merP", "lgrE", "dat", "acrE", 
                                 "4224471", "inh", "4787153",
                                 "5067956", "5071406", "5126084", 
                                 "5171225", "5204149", "5606105", 
                                 "ftsA", "5611644", "5612036", "hin", 
                                 "virD4", "5628279", "5643301", 
                                 "1145152", "2702075",  "2710069", "3055598", 
...
heatmap_genes_4h_sense <- pheatmap(logcpm_sense_df_h, angle_col = "45", fontsize = 8, cellwidth = 20, 
                                   ...
                                   breaks = breaksList,
                                   show_colnames = T, cutree_cols = 2, legend = F, treeheight_col = 5,
                                   treeheight_row = 0)
...
ggsave(heatmaps, file="save_figures/Figure_09.tif", device="tiff", 
       dpi=300, width=25, height=25, units = "cm", bg = "white")
dev.off()
